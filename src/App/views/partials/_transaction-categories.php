        ...
        <div id="settings-income-categories-list" class="settings-category-list">
            <?php if (!empty($incomeCategories)) : ?>
                <?php foreach ($incomeCategories as $incomeCategory) : ?>
                    <form action="/settings/income-category/<?php echo e($incomeCategory['income_category_assigned_to_user_id']); ?>" method="POST">
                        <input type="hidden" name="_METHOD" value="DELETE" />
                        <?php include $this->resolve('partials/_csrf.php'); ?>
                        <div class="settings-single-category-catch-mouse" style="display: flex; flex-direction:row; align-items:center">
                            <span class="settings-single-category"><?php echo $incomeCategory['income_category_name'] ?></span>
                            <button class="settings-category-delete-button" type="submit" hidden>
                ...
                <?php endforeach; ?>
            <?php else : ?>
                <p style="margin: 5px 5px 5px 5px;">No categories available</p>
            <?php endif; ?>
        </div>
    </div>
    <form action="/settings-add-new-income-category" method="POST" class="settings-add-new-category">
        ...
        <div style="display: flex; flex-direction:column;">
            ...
            <div style="display: flex; flex-direction:row;">
                <input type="text" name="new-income-category" class="settings-data-input <?php echo array_key_exists('new-income-category', $errors) ? 'settings-data-input-error' : ''; ?>">
...
<div class="settings-data" style="margin: 0px 5px 0px 5px;">
        ...
        <div id="settings-expense-categories-list" class="settings-category-list">
            <?php if (!empty($expensesCategories)) : ?>
                <?php foreach ($expensesCategories as $expenseCategory) : ?>
                    <form action="/settings/expense-category/<?php echo e($expenseCategory['expense_category_assigned_to_user_id']); ?>" method="POST">
                        <input type="hidden" name="_METHOD" value="DELETE" />
                        <?php include $this->resolve('partials/_csrf.php'); ?>
                        <div class="settings-single-category-catch-mouse" style="display: flex; flex-direction:row; align-items:center">
    ...
    <form action="/settings-add-new-expense-category" method="POST" class="settings-add-new-category">
        ...
        <div style="display: flex; flex-direction:column;">
            ...
            <div style="display: flex; flex-direction:row;">
                <input type="text" name="new-expense-category" class="settings-data-input <?php echo array_key_exists('new-expense-category', $errors) ? 'settings-data-input-error' : ''; ?>">
                <input type="submit" value="Add" class="settings-data-button">
            </div>
